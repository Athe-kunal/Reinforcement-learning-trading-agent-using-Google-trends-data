...
class PerformanceEstimator:
    ...
    def __init__(self, tradingData):
        ...
        # self.data['Money'] = pd.Series(self.data['Money'].values)
        # self.data['Returns'] = pd.Series(self.data['Returns'].values)
    ...
    def computePnL(self):
        ...
        # Compute the PnL
        self.PnL = self.data["Money"].values[-1] - self.data["Money"].values[0]
        return self.PnL
    ...
    def computeAnnualizedReturn(self):
        ...
        cumulativeReturn = self.data['Returns'].values.cumsum()
    ...
    def computeAnnualizedVolatility(self):
        ...
        self.annualizedVolatily = 100 * \
            np.sqrt(252) * self.data['Returns'].values.std()
    ...
    def computeSharpeRatio(self, riskFreeRate=0):
        ...
        expectedReturn = self.data['Returns'].values.mean()
        
        # Compute the returns volatility
        volatility = self.data['Returns'].values.std()
    ...
    def computeSortinoRatio(self, riskFreeRate=0):
        ...
        # Compute the expected return
        expectedReturn = np.mean(self.data['Returns'].values)
        
        # Compute the negative returns volatility
        negativeReturns = [
            returns for returns in self.data['Returns'].values if returns < 0]
    ...
    def computeMaxDrawdown(self, plotting=False):
        ...
        if plotting:
            ...
            plt.plot(self.data['Money'].values, lw=2, color='Blue')
